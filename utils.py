...
from networks import (MLP, VGG11, VGG11BN, AlexNet, AlexNetBN, ConvNet, LeNet,
                      LeNet5, ResNet18, ResNet18BN, ResNet18BN_AP)
...
def evaluate_synset(net: nn.Module, images_train: torch.Tensor, labels_train: torch.Tensor, 
                    ...
                    device: torch.device, lr: float, num_epochs: int, batch_size: int):
    net = net.to(device)
    ...
    print('%s Evaluate_%02d: epoch = %04d train time = %d s train loss = %.6f train acc = %.4f, test acc = %.4f' % (get_time(), 0, num_epochs, t.time, loss_train, acc_train, acc_test))
